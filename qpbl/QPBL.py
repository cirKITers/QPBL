...
This module implements the Eigen and Test operations used in
Quantum Phase-Based Learning (QPBL).
"""
...
class Eigen(Operation):
    ...
    def compute_decomposition(
        ...
        for i in range(LQubits):
            ...
            op_list.append(
                qml.ctrl(
                    qml.QubitUnitary(Utils.powm(O, i), wires=range(offset_U, n_qubits)),
                    control=offset_U - 1 - i,
                )
            )
...
class Mottonen(Operation):
    ...
    def __init__(
        self, theta: np.ndarray, wire=None, QQubits: int = 2, offset_U: int = 5
    ):
        self._hyperparameters = {
            "QQubits": QQubits,
            "offset_U": offset_U,
    ...
    @staticmethod
    def compute_decomposition(
        theta: np.ndarray, wires, QQubits: int = 2, offset_U: int = 5
    ) -> list[Operation]:
        op_list = []
        c = 2**QQubits - 2
        for k, i in enumerate(range(QQubits - 1, -1, -1)):
            for j in range(2**k - 1, -1, -1):
                if k > 0:
                    op_list.append(
                        qml.ctrl(
                            qml.adjoint(qml.RY(theta[c], wires=k + offset_U)),
                            control=range(offset_U, k + offset_U),
                            control_values=Utils.int_to_bin(j, k),
                        )
                    )
                else:
